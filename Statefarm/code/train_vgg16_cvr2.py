...
net['new_output'] = DenseLayer(net['drop7'], num_units=10, nonlinearity=softmax, W=lasagne.init.HeNormal(0.01)) 
...
X, y, drivers = load_train_data('../input', grayscale=False, img_shape=IMG_SHAPE, usecache=False)
...
kf = StratifiedKFold(y, n_folds=8, shuffle=True, random_state=1)
...
for i, (tr_ix, val_ix) in enumerate(kf):
    ...
    for epoch in range(3):
        ...
        kf2 = KFold(len(y_tr), n_folds=np.floor(len(y_tr)/BATCH_SIZE), shuffle=True, random_state=epoch)
    ...
    make_submission('../output/submission_vgg16_2' + str(i) + '.csv', y_proba, ids)
