...
net['new_output'] = DenseLayer(net['fc7'], num_units=10, nonlinearity=softmax)
...
learning_rate = theano.shared(np.array(0.002, dtype=theano.config.floatX))
learning_rate_decay = np.array(0.96, dtype=theano.config.floatX)
updates = {}
...
def train_batch():
    ...
    X_new = np.zeros_like(X_tr[ix])
    for i, x in enumerate(X_tr[ix]):
        if p >= 0.5 and p < 0.8:
            # rotate
            angle = (np.random.rand() - 0.5) * 2 * 6
            X_new[i] = rotate(x, angle, mode='nearest', reshape=False)
        elif p >=0.8:
            channel = np.floor(np.random.rand() * 2.999999)
            X_new[i,channel] = x[channel] + (np.random.rand() - 0.5) * 20
    '''
    return train_fn(X_new, y_tr[ix])
...
for i, (tr_ix, val_ix) in enumerate(train_split):
    print('CV Fold', i)
    X_tr = X[tr_ix]
    y_tr = y[tr_ix]
    X_val = X[val_ix]
    ...
    net['new_output'] = DenseLayer(net['fc7'], num_units=10, nonlinearity=softmax, W=lasagne.init.Normal(0.01)) 
    lasagne.layers.set_all_param_values(net['prob'], d['param values'])
    learning_rate.set_value(0.002)
    ...
    for epoch in range(25):
        ...
        val_acc_tot = 0.
        '''
        for chunk in batches(t_ix, BATCH_SIZE):
        ...
        '''
        tr_loss_tot /= len(t_ix)
        tr_acc_tot /= len(t_ix)
        '''
        val_loss_tot /= len(v_ix)
        val_acc_tot /= len(v_ix)
        #print(epoch, tr_loss_tot, tr_acc_tot * 100, val_loss_tot, val_acc_tot * 100)
        print(epoch, val_loss_tot, val_acc_tot * 100)

        # at epoch 20 set a benchmark for early stopping
        if epoch == 20:
            benchmark = val_loss_tot
        if epoch > 20 and val_loss_tot < (benchmark * 0.8):
	        print("nice epoch... early stopping...")
	        break
    ...
    make_submission('../output/submission_f' + str(i) + '.csv', y_proba, ids)
