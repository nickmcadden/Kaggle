...
d = pickle.load(open('../input/pretrained/vgg_cnn_s.pkl'))
...
net['new_output'] = DenseLayer(net['drop7'], num_units=10, nonlinearity=softmax)
...
learning_rate = theano.shared(np.array(0.0005, dtype=theano.config.floatX))
learning_rate_decay = np.array(0.4, dtype=theano.config.floatX)
updates = OrderedDict()
...
for i, (tr_ix, val_ix) in enumerate(kf):
    ...
    lasagne.layers.set_all_param_values(net['prob'], d['values'])
    learning_rate.set_value(0.0005)
    ...
    for epoch in range(5):
        ...
        if epoch > 45 and val_loss_tot < (benchmark * 0.8):
    ...
    make_submission('../output/submission_s' + str(i) + '.csv', y_proba, ids)
