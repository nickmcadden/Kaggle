...
setwd("/Users/nmcadden/Desktop/Kaggle/NBA/")

###############################################################
#Submission is set as Jeff's sample submission. 
#Change this to your submission file.
###############################################################
submission<-read.csv("/Users/nmcadden/ncaa_submission_f3.csv")

teams<-read.csv("/Users/nmcadden/Desktop/Kaggle/NBA/teams.csv")

###############################################################
#Seeds will probably need to be updated when Jeff gets out the new
#seedings on Sunday or Monday.
###############################################################
seeds<-read.csv("/Users/nmcadden/Desktop/Kaggle/NBA/tourney_seeds_2015.csv")
...
round2 <- rbind(subw1[c(5:8)],subx1[c(5:8)],suby1[c(5:8)],subz1[c(5:8)])
setwd("/Users/nmcadden/Desktop/Kaggle/NBA/")
write.csv(round2, file = "round2.csv",quote=FALSE,row.names=FALSE)

spread_2015 <- as.data.frame(read.csv("/Users/nmcadden/Desktop/Kaggle/NBA/round2.csv"))
