...
clf =  GradientBoostingClassifier(loss='deviance', learning_rate=0.02, n_estimators=500, subsample=0.65, min_samples_split=6, min_samples_leaf=1,  max_depth=3, random_state=1, verbose=1)
...
oob_pred_filename = '../output/oob_pred_gbcentropy_' + str(np.mean(scr))
sub_pred_filename = '../output/sub_pred_gbcentropy_' + str(np.mean(scr))
...
preds.to_csv('../output/gbc_' + str(np.mean(scr)) + '.csv', index=False)
