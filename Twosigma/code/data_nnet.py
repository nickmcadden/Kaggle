...
def load(m_params):
	...
	train["bedrooms"] = np.clip(train["bedrooms"],1,10)
	train["bathrooms"] = np.clip(train["bathrooms"],1,5)
	test["bedrooms"] = np.clip(test["bedrooms"],1,10)
	test["bathrooms"] = np.clip(test["bathrooms"],1,5)

	train["price_per_bed"] = np.log1p(train["price"]/train["bedrooms"])
	train["price_per_bath"] = np.log1p(train["price"]/train["bathrooms"])
	train["room_diff"] = train["bedrooms"]-train["bathrooms"]
	train["room_sum"] = 1 + train["bedrooms"]+train["bathrooms"]/2
	train["price_per_room"] = np.log1p(train["price"]/train["room_sum"])
	train["beds_percent"] = np.log1p(train["bedrooms"]/train["room_sum"])

	test["price_per_bed"] = np.log1p(test["price"]/test["bedrooms"])
	test["price_per_bath"] = np.log1p(test["price"]/test["bathrooms"])
	test["room_diff"] = test["bedrooms"]-test["bathrooms"]
	test["room_sum"] = 1 + test["bedrooms"]+test["bathrooms"]/2
	test["price_per_room"] = np.log1p(test["price"]/test["room_sum"])
	test["beds_percent"] = np.log1p(test["bedrooms"]/test["room_sum"])
	...
	train["latlon_bed"] = train["latlon"]*100000 + train["bedrooms"]
	test["latlon_bed"] = test["latlon"]*100000 + test["bedrooms"]
	...
	categorical = ["manager_id", "building_id", "geo_cluster"]

	categorical_new = ["manager_id_count", "building_id_count", "geo_cluster_count"]
	...
	train, test = add_median_price(key=["geo_clust_bed"], suffix="bed", trn_df=train, tst_df=test)
	train, test = add_median_price(key=["latlon_bed"], suffix="latlon_bed", trn_df=train, tst_df=test)
	...
	date_features = ["created_month", "created_day", "created_hour", "days_since"] # "created_weekday"
	...
	other_features = ["num_photos", "east", "west", "price_to_median_ratio_bed", "price_to_median_ratio_latlon_bed"] # "cool_door_num", "east", "west", "price_to_median_ratio_geo", "building_door_num"

	for i in train.columns:
		print(i, train[i].dtype)

	all_features = original_features + date_features + room_features + word_features + other_features + categorical

	print(all_features)

	train["price"] = np.log(train["price"])
	test["price"] = np.log(test["price"])
	train["manager_id_count"] = np.clip(np.log(train["manager_id_count"]),1,100)
	test["manager_id_count"] = np.clip(np.log(test["manager_id_count"]),1,100)
	train["building_id_count"] = np.clip(np.log(train["building_id_count"]),1,100)
	test["building_id_count"] = np.clip(np.log(test["building_id_count"]),1,100)
	train["geo_cluster_count"] = np.clip(np.log(train["geo_cluster_count"]),1,100)
	test["geo_cluster_count"] = np.clip(np.log(test["geo_cluster_count"]),1,100)
	...
	'''
	return train.todense(), y.values, test.todense(), ids
