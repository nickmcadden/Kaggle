...
def get_photo_id_diff(id, photo_list):
	if len(photo_list):
		return int(int(photo_list[0].split('/')[4].split('_')[0]) != int(id))
	else:
		return 0
...
def load(m_params):
	...
	train["photo_id_diff"] = train.apply(lambda row: get_photo_id_diff(row['listing_id'], row['photos']), axis=1)
	test["photo_id_diff"] = test.apply(lambda row: get_photo_id_diff(row['listing_id'], row['photos']), axis=1)
	...
	tfidf = CountVectorizer(stop_words='english', max_features=75)
	...
	other_features = ["photo_id_diff", "num_photos", "price_to_median_ratio_bed", "price_to_median_ratio_latlon_bed"] # "cool_door_num", "east", "west", "building_door_num"
	...
	train_log = np.log1p(np.clip(train,0,1000000))
	test_log = np.log1p(np.clip(test,0,1000000))

	train_sqr = np.power(train, 2)
	test_sqr = np.power(test, 2)
	
	train_sqrt = np.power(train, 0.5)
	test_sqrt = np.power(test, 0.5)

	print(train.shape)
	train = pd.concat([train, train_log, train_sqr, train_sqrt], axis=1)
	test = pd.concat([test, test_log, test_sqr, train_sqrt], axis=1)
	print(train.shape)
