...
def load(m_params):
	...
	interest_low = pd.Series(train['interest_level'] == 2)
	interest_med = pd.Series(train['interest_level'] == 1)
	interest_high = pd.Series(train['interest_level'] == 0)
	...
	train["img_delta"] = (train["img_date"] - train["created"]).astype("timedelta64[D]").astype(int)
	test["img_delta"] = (test["img_date"] - test["created"]).astype("timedelta64[D]").astype(int)
	...
	original_features = ["price", "room_sum", "bathrooms", "bedrooms", "listing_id"]
	img_date_features = ["img_delta", "img_days_passed", "img_date_month", "img_date_week", "img_date_day", "img_date_dayofweek", "img_date_dayofyear", "img_date_hour", "img_date_monthBeginMidEnd"]

	for i in train.columns:
		print(i, train[i].dtype)

	all_features = original_features + img_date_features

	train.fillna(0, inplace=True)
	test.fillna(0, inplace=True)

	train = train[all_features].values
	test = test[all_features].values
