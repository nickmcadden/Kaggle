...
def categorical_average(variable, y, pred_0, feature_name):
    ...
    #cv for training set 
    k_fold = StratifiedKFold(X_train[y], n_folds=5, shuffle=True, random_state=1)
    X_train[feature_name] = -999 
    for (train_index, cv_index) in k_fold:
...
def prepare_submission(model):
    ...
    sub['medium'] = preds[:, 1]
    sub['high'] = preds[:, 2]
    sub.to_csv("../output/xgb_0.534637373771.csv", index = False, header = True)
