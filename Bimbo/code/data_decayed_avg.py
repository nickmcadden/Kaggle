...
def load(m_params):
	...
	print('product client decayed...')
	prod_client_decayed_avg = pd.read_csv(inputdir + 'prod_client_decayed_avg_preds.csv')
	...
	train = pd.merge(train, prod_client_decayed_avg, how='left', on=["ProductId","ClientId"])
	...
	test = pd.merge(test, prod_client_decayed_avg, how='left', on=["ProductId","ClientId"])
	...
	test_ids = test['id']
	train.drop(['WeekNum','ProdClientLastWeekUnits','SalesUnitsWeek','SalesPesosWeek','ReturnsUnitsWeek', 'ReturnsPesosWeek', 'AdjDemand','DepotId', 'ChannelId', 'RouteId', 'ClientId', 'ProductId'], axis=1, inplace=True)
	test.drop(['WeekNum','id','DepotId','ProdClientLastWeekUnits','ChannelId', 'RouteId', 'ClientId', 'ProductId'], axis=1, inplace=True)
	...
	train['week2'] = train['week2'].fillna(-1)
	test['week2'] = test['week2'].fillna(-1)
