...
print(X_train[:100,7:19])

print("Bimbo: XGB regression...\n")
xgb_param = {'silent' : 1, 'missing' :-1, 'max_depth' : 7, 'eval_metric': 'rmse', 'objective': 'reg:linear', 'eta': m_params['eta'], 'min_child_weight': 2, 'subsample': 0.7, 'colsample_bytree': 0.5}
...
if m_params['cv']:
	...
	'''
	for i in range(7,10):
		pred = X_val[:,i].astype(np.float32)
		goodidx = (np.isnan(pred) == 0)
		scr = np.sqrt(mean_squared_error(np.log1p(y_val[goodidx]), np.log1p(pred[goodidx])))
		print('col score is:', scr, np.sum(goodidx))
	'''
	...
	model_pathname = '../output/pred_xgb_simple' + str(m_params['n_rounds'])
