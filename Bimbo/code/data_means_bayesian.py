...
def load(m_params):
	...
	product_detail  = pd.read_csv('../input/product_attr.csv')
	'''
	product_detail.drop(['idx','product_name'], inplace=True, axis=1)
	product_detail = product_detail.fillna(1)
	lbl = LabelEncoder()
	lbl.fit(product_detail['brand'])
	product_detail['brand'] = lbl.transform(product_detail['brand'])
	'''	
	train = pd.merge(train, product_detail, how='left', on="ProductId")
	test = pd.merge(test, product_detail, how='left', on="ProductId")
	...
	print('product client agent route ...')
	prod_client_agent_route_mean = pd.read_csv(inputdir + 'groupby_product_client_agent_route_mean_units.csv')
	print('product client agent...')
	prod_client_agent_mean = pd.read_csv(inputdir + 'groupby_product_client_agent_mean_units.csv')
	print('product client...')
	prod_client_mean = pd.read_csv(inputdir + 'groupby_product_client_mean_units.csv')
	...
	print('product route...')
	prod_route_med = pd.read_csv(inputdir + 'groupby_product_route_mean_units.csv')
	print('product depot...')
	prod_depot_med = pd.read_csv(inputdir + 'groupby_product_depot_mean_units.csv')
	...
	#train = pd.merge(train, prod_client_agent_route_mean, how='left', on=["ProductId","ClientId","DepotId","RouteId"])
	#train = pd.merge(train, prod_client_agent_mean, how='left', on=["ProductId","ClientId","DepotId"])
	...
	#test = pd.merge(test, prod_client_agent_route_mean, how='left', on=["ProductId","ClientId","DepotId","RouteId"])
	#test = pd.merge(test, prod_client_agent_mean, how='left', on=["ProductId","ClientId","DepotId"])
	...
	train['hasProdC50'] = np.isfinite(train['ProdC50Units'])
	test['hasProdC50'] = np.isfinite(test['ProdC50Units'])
	...
	test_ids = test['id']
	train.drop(['c1000','c250','c50','pc10','pc100','pc30','ProdClientLastWeekUnits','SalesUnitsWeek', 'SalesPesosWeek','ReturnsUnitsWeek', 'ReturnsPesosWeek', 'AdjDemand','DepotId', 'ChannelId', 'RouteId', 'ClientId'], axis=1, inplace=True)
	test.drop(['c1000','c250','c50','pc10','pc100','pc30','id','DepotId', 'ProdClientLastWeekUnits', 'ChannelId', 'RouteId', 'ClientId'], axis=1, inplace=True)
	...
	#train['ProdClientAgentUnits'] = train['ProdClientAgentUnits'].fillna(train['ProdClientUnits'])
	#test['ProdClientAgentUnits'] = test['ProdClientAgentUnits'].fillna(test['ProdClientUnits'])
	#train['ProdClientAgentRouteUnits'] = train['ProdClientAgentRouteUnits'].fillna(train['ProdClientAgentUnits'])
	#test['ProdClientAgentRouteUnits'] = test['ProdClientAgentRouteUnits'].fillna(test['ProdClientAgentUnits'])	
	
	train['ProdC50Units'] = 0.5*train['ProdC50Units'] + 0.5*train['ProdUnits']
	train['ProdC250Units'] = 0.5*train['ProdC250Units'] + 0.5*train['ProdC50Units']
	train['ProdC1000Units'] = 0.5*train['ProdC1000Units'] + 0.5*train['ProdC250Units']
	train['ProdClientUnits'] = 0.5*train['ProdClientUnits'] + 0.5*train['ProdC1000Units']

	test['ProdC50Units'] = 0.5*test['ProdC50Units'] + 0.5*test['ProdUnits']
	test['ProdC250Units'] = 0.5*test['ProdC250Units'] + 0.5*test['ProdC50Units']
	test['ProdC1000Units'] = 0.5*test['ProdC1000Units'] + 0.5*test['ProdC250Units']
	test['ProdClientUnits'] = 0.5*test['ProdClientUnits'] + 0.5*test['ProdC1000Units']
