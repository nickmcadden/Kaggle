...
def homedepot_dict_lookup(trainnn):
	t = []
	trainnn = pd.DataFrame(trainnn)
	for i in trainnn.index:
		for j in trainnn.ix[i,3].split():
			#t.append((stemmer.stem(j.lower()), train.ix[i,4]))
			t.append((j, trainnn.ix[i,4]))
...
def get_term_score_freq(words):
	...
	for word in words:
		try:
			term_score += lookup.ix[word,'score']
			term_freq += lookup.ix[word,'freq']
		except:
			term_score += np.mean(lookup['score'])
