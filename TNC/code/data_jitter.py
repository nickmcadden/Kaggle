...
def _load_img(file, img_shape=(224, 224), swapaxes=True, normalize=False):
    ...
    img = imread(file)
    #img = exposure.adjust_gamma(img, 1+(np.random.rand()-0.5)*0.5)
    y1_crop = np.int(np.random.rand() * 50) + 1
    y2_crop = np.int(np.random.rand() * 50) + 1
    x1_crop = np.int(np.random.rand() * 50) + 1
    x2_crop = np.int(np.random.rand() * 50) + 1
