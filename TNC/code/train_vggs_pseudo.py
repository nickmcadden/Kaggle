...
from data_pretrained import load_train_data, load_test_data, load_pseudo_labels, make_submission
...
net['new_output'] = DenseLayer(net['drop7'], num_units=8, nonlinearity=softmax)
...
X, y, ids = np.concatenate((X_train, X_test)), np.concatenate((y_train, y_pseudo)), np.concatenate((ids_train, ids_test))
...
for i, (tr_ix, val_ix) in enumerate(kf):
    ...
    for epoch in range(2):
        ...
        print("loading new pseudo labels")
        y[np.array(new_pseudo_label_pos)] = load_pseudo_labels()
    ...
    make_submission('../output/submission_vgs_' + str(i) + '.csv', y_proba, ids)
