...
def _load_img(file, img_shape=(299, 299)):
    #shape = list(img_shape) + [3]
    img = imread(file)
    img = sk_transform.resize(img, img_shape, preserve_range=True)

    img = (img - 128) / 128.
    img = np.rollaxis(img, 2)[np.newaxis].astype('float32')
    return img
...
def load_train_data(img_shape=(299,299), rotate=False, display=False):
    X = []
    X_id = []
    y = []
...
def load_test_data(img_start_ix=0, max_img=100, img_shape=(299,299)):
