...
oob_pred = np.zeros((X.shape[0], 8)).astype(float)
...
for i, (tr_ix, val_ix) in enumerate(kf):
    ...
    for epoch in range(1):
        ...
        for chunk in batches(val_ix, BATCH_SIZE):
            oob_pred[chunk] = pred_fn(X[chunk]).astype(float)
    ...
    imgs_per_batch = 50
    for j in range(0, 1000, imgs_per_batch):
    ...
    print(cmatrix(y, np.argmax(oob_pred, axis=1)))
