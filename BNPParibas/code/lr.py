...
clf = LogisticRegression(penalty='l1', C=0.025)
...
if m_params['cv']:
	...
	for i, (tr_ix, val_ix) in enumerate(kf):
		...
		oob_pred[val_ix] = np.clip(np.array(pred[:,1]),0.0001,0.9999)
	...
	oob_pred_filename = '../output/oob_pred_logit_' + str(np.mean(scr))
	sub_pred_filename = '../output/sub_pred_logit_' + str(np.mean(scr))
