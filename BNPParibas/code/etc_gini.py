...
clf = ExtraTreesClassifier(n_estimators=m_params['n_rounds'], max_features= 50, criterion= 'gini', min_samples_split= 4, max_depth= 35, min_samples_leaf= 2, n_jobs = -1)      
...
if m_params['cv']:
	...
	oob_pred_filename = '../output/oob_pred_etcgini_' + str(np.mean(scr))
	sub_pred_filename = '../output/sub_pred_etcgini_' + str(np.mean(scr))
