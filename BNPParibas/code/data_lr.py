...
def load(m_params):
	...
	for c in train_continuous:
		train_continuous[c+'_inv'] = 1 / np.clip(train[c], 0.0001, 10000000000)
		test_continuous[c+'_inv']  = 1 / np.clip(test[c], 0.0001, 10000000000)
		train_continuous[c+'^2'] = np.power(train[c], 2)
		test_continuous[c+'^2'] = np.power(test[c], 2)


	print("\n\n2. Bin vars...\n")
	bincols = ['v50','v10','v12','v34']
	for c in bincols:
		train_continuous[c], test_continuous[c] = bin(train_continuous[c], test_continuous[c], labels, 1500)
	train_continuous.ix[:, bincols], test_continuous.ix[:, bincols] = category_transformation(train_continuous.ix[:, bincols], test_continuous.ix[:, bincols], labels, type='tgtrate')
	...
	print("train categoric shape: ", train_categoric.shape)
