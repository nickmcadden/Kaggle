...
def prob_weight_lookup2(code, lookup, labels, target=np.nan):
	try:
		if target == 0:
			scr = np.random.normal(loc=lookup.ix[code, 'tgt0'], scale=lookup.ix[code, 'ci']) 
		elif target == 1:
			scr = np.random.normal(loc=lookup.ix[code, 'tgt1'], scale=lookup.ix[code, 'ci']) 
		else:
...
def category_to_prob_weight2(train, test, col, labels):
	...
	lookup = pd.merge(corr, freqs, how='outer', left_index=True, right_index=True)
	lookup.columns = (['target','freq'])  
	lookup['tgt_rate_adj'] = ((lookup['freq'] * lookup['target']) + np.mean(labels) * 30) / (lookup['freq'] + 30)
	lookup['tgt1'] = ((lookup['freq'] * lookup['target'] - 1) + np.mean(labels) * 30) / (lookup['freq'] + 29)
	lookup['tgt0'] = ((lookup['freq'] * lookup['target']) + np.mean(labels) * 29) / (lookup['freq'] + 29)
	'''  
	lookup['tgt1'] = np.clip(((lookup['target'] * lookup['freq']) - 1) / (lookup['freq'] - 1), 0, 1)
	lookup['tgt1'] = ((lookup['freq'] - 1) * lookup['tgt1'] + np.mean(labels) * 10) / (lookup['freq'] + 9)
	lookup['tgt0'] = np.clip(((lookup['target'] * lookup['freq'])) / (lookup['freq'] - 1), 0, 1)
	lookup['tgt0'] = ((lookup['freq'] - 1) * lookup['tgt0'] + np.mean(labels) * 10) / (lookup['freq'] + 9)
	'''
	lookup.ix[lookup['tgt1'].isnull(), 'tgt1'] = labels.mean()
	lookup.ix[lookup['tgt0'].isnull(), 'tgt0'] = labels.mean()
	lookup['ci'] = 1 * np.sqrt((lookup['tgt_rate_adj'] * (1-lookup['tgt_rate_adj']) / (lookup['freq']+30)))
	print(lookup)
...
def category_transformation(train_categoric, test_categoric, labels, type='std'):
	...
	if type == 'tgtrate':
		...
		for c in train_categoric.columns:
			train_categoric[c], test_categoric[c] = category_to_prob_weight2(train_categoric, test_categoric, c, labels)
...
def load(m_params):
	...
	train_decomp, test_decomp = category_transformation(train_categoric, test_categoric, labels, type = 'std')
	train_decomp = pd.DataFrame(decomp.fit_transform(train_decomp))
	test_decomp = pd.DataFrame(decomp.transform(test_decomp))

	train_categoric, test_categoric = category_transformation(train_categoric, test_categoric, labels, type = m_params['cat_trans'])
	'''
	train_categoric['cat_mean'] = train_categoric.median(axis=1)
	test_categoric['cat_mean'] = test_categoric.median(axis=1)
	'''
	print("\n\n3. Numeric column normalisation... \n")
	numeric_col_count = 0
	for c in train_continuous.columns:
		train_continuous[c], test_continuous[c] = np.log(train_continuous[c]+2), np.log(test_continuous[c]+2)
		numeric_col_count += 1
	...
	train = pd.concat([train_categoric, train_discrete, train_continuous, train_decomp], axis=1)
	test = pd.concat([test_categoric, test_discrete, test_continuous, train_decomp], axis=1)
