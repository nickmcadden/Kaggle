...
def category_transformation(train_categoric, test_categoric, labels, type='std'):
	...
	if type == 'std':
		...
		for c in train_categoric.columns:
			train_categoric[c], tmp_indexer = pd.factorize(train_categoric[c])
			test_categoric[c] = tmp_indexer.get_indexer(test_categoric[c])
			'''
			lbl = LabelEncoder()
...
def load(m_params):
	...
	for c in train_continuous.columns:
		train_continuous.loc[:, c], test_continuous.loc[:, c] = np.log(train_continuous.loc[:, c]+2), np.log(test_continuous.loc[:, c]+2)
		numeric_col_count += 1
		#if not (numeric_col_count % 10):
			#print('Numeric Col Count: ', numeric_col_count)

	for c in train_discrete.columns:
		train_discrete.loc[:, c], test_discrete.loc[:, c] = np.log(train_discrete.loc[:, c]+2), np.log(test_discrete.loc[:, c]+2)
		numeric_col_count += 1
		#if not (numeric_col_count % 10):
			#print('Numeric Col Count: ', numeric_col_count)
	...
	print("Numeric Column Smoothing... \n\n")
	if minbin > 1:
		for c in train_continuous.columns:
