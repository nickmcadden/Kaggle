...
if __name__ == '__main__':
    ...
    if test_run:
        ...
        for run in range(1, 11):
            ...
            tr_preds1=xgb_learning(tr_train_y, tr_train, tr_test)
            tr_preds2=linear_learning(tr_train_y, tr_train, tr_test)
            tr_preds3=svm_learning(tr_train_y, tr_train, tr_test)
            tr_preds4=random_learning(tr_train_y, tr_train, tr_test)

            rmse_score1 = (np.sum((np.log1p(tr_preds1)-np.log1p(tr_test_y))**2)/len(test_i))**0.5
			rmse_score2 = (np.sum((np.log1p(tr_preds2)-np.log1p(tr_test_y))**2)/len(test_i))**0.5
			rmse_score3 = (np.sum((np.log1p(tr_preds3)-np.log1p(tr_test_y))**2)/len(test_i))**0.5
			rmse_score4 = (np.sum((np.log1p(tr_preds4)-np.log1p(tr_test_y))**2)/len(test_i))**0.5
            ...
            print ("logistic regression score for test run %i is %.6f %.6f %.6f %.6f" %(run, rmse_score1, rmse_score2, rmse_score3, rmse_score4))
        print ("Mean RMSE1 is %.6f:" %np.mean(rmse1))
		print ("Mean RMSE2 is %.6f:" %np.mean(rmse2))
		print ("Mean RMSE3 is %.6f:" %np.mean(rmse3))
		print ("Mean RMSE4 is %.6f:" %np.mean(rmse4))
    else:
