...
# function to split training data into stratified set of n folds
stratifiedkfold <- function(df,nfold) {
  nfold<-5
  idxs_fold <- numeric(0)
  classdist <- table(train_raw$target)
  classcount <- nrow(classdist)
  folddist <- floor(classdist/nfold)
  for (i in 1:classcount) {
    idxs <- which(train_raw$target == rownames(classdist)[i])
    for (j in 1:nfold) {
      a<-(j-1)*(folddist[i])+1
      b<-j*folddist[i]
      idxs_fold <- c(idxs_fold, idxs[a:b])
    }
  }
  return(idxs_fold)
}

a<-stratifiedkfold(train_raw,5,3)
